<script lang="ts">
	...
	let roleIcon = $derived(ROLES.find((r) => r.name === character.role)?.icon ?? '');
...
<header class="sheet-header">
	...
	<div class="header-fields">
		...
		<div class="field-group origin-group">
			<button id="origin" class="origin-dropdown" onclick={() => (showOriginModal = true)}>
		...
		<div class="field-group role-group">
			<button id="role" class="origin-dropdown" onclick={() => (showRoleModal = true)}>
				{roleIcon ? `${roleIcon} ` : ''}{character.role || 'Select Role...'}
			</button>
		...
		<div class="field-group movement-group">
			<input
				...
				id="movement"
				readonly
				value={character.origin ? character.movement : ''}
			/>
