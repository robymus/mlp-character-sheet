<script lang="ts">
    ...
    import { GENERAL_PERKS, type GeneralPerk } from '$lib/data/generalPerks';
    ...
    function checkPrerequisite(perk: GeneralPerk): { met: boolean; reason: string } {
        ...
        if (perk.prerequisite.essence) {
            for (const [ess, minVal] of Object.entries(perk.prerequisite.essence)) {
                const current = character.essenceValue(
                    ess as 'strength' | 'speed' | 'smarts' | 'social',
                );
                if (current < minVal!) {
        ...
        if (perk.prerequisite.skill) {
            for (const [skill, minRank] of Object.entries(perk.prerequisite.skill)) {
                ...
                if (current < minRank) {
                    const dieLabel =
                        ['', 'd2', 'd4', 'd6', 'd8', 'd10', 'd12'][minRank] || `rank ${minRank}`;
    ...
    <div class="perks-container">
        <div class="perk-section">
            <div class="perk-item read-only">
                ...
                <p class="perk-desc italic-note">
                    Please discuss your cutie mark perk with your GM.
                </p>
            </div>
        </div>
        ...
        <div class="perk-section">
            ...
            {:else if character.origin === 'Pegasus'}
                <div class="perk-item read-only origin-box" style="margin-bottom: 5px;">
                    ...
                    <p class="perk-desc">
                        You were born with wings but without the strength to use them. Movement
                        speed:
                        <select
                ...
                <div class="perk-item read-only origin-box">
                    ...
                    <p class="perk-desc">
                        Aerial acrobatics are in your hollow bones. You get ↑1 on Acrobatics Skill
                        Tests. You can land on clouds and walk around Pegasus sky cities.
                    </p>
                </div>
            {:else if character.origin === 'Unicorn'}
                <div class="perk-item read-only origin-box">
                    ...
                    <p class="perk-desc">
                        Unicorns learn to use their telekinetic abilities to manipulate objects at a
                        distance, which makes their horn glow with magical power.
                    </p>
                </div>
                <div class="perk-item read-only origin-box">
                    <span class="perk-name">Magical</span>
                    <p class="perk-desc">
                        You are a magical pony capable of casting spells. Unlocks the Spellcasting
                        skill and spell slots.
                    </p>
                </div>
            {:else}
        ...
        {#if character.origin === 'Earth Pony'}
            <div class="perk-section">
                ...
                {#each character.generalPerks as perkName, i (perkName + '-' + i)}
                    ...
                    {@const prereqCheck = perk
                        ? checkPrerequisite(perk)
                        : { met: true, reason: '' }}
                    {#if perk}
                        <div
                            class="perk-item added-perk-box"
                            class:prereq-warning={!prereqCheck.met}
                        >
                            <div class="perk-item-header">
                                <span class="perk-name">
                                    {#if !prereqCheck.met}
                                        <span class="warning-icon" title={prereqCheck.reason}
                                            >⚠️</span
                                        >
                                    {/if}
                                    {perk.name}
                                </span>
                                <button class="remove-btn" onclick={() => removeGeneralPerk(i)}
                                    >&times;</button
                                >
                            </div>
                            <p class="perk-desc">
                            ...
                            {#if perk.choices}
                                <div class="perk-choice">
                                    ...
                                    <select
                                        ...
                                        onchange={(e) =>
                                            setChoice(perk.name, e.currentTarget.value)}
                                    >
        ...
        {#if character.role}
            ...
            <div class="perk-section" style="margin-top: 15px;">
                ...
                <div class="perk-item read-only">
                    ...
                    <p class="perk-desc">
                        Once per scene, when you act in the spirit of {roleName}, you gain a
                        Friendship Point.
                    </p>
                </div>
                ...
                <div class="perk-item read-only">
                    ...
                    <p class="perk-desc">
                        Once per scene, a pony in your group can spend a Friendship Point as a
                        Standard action to form a Friendship Circle.
                    </p>
                </div>
                ...
                {#if roleName === 'Generosity'}
                    <div class="perk-item read-only">
                        ...
                        <p class="perk-desc">
                            You can grant another player character ↑1 to any Skill Test before they
                            roll the dice. You suffer ↓1 on your next Skill Test.
                        </p>
                    </div>
                {:else if roleName === 'Honesty'}
                    <div class="perk-item read-only">
                        ...
                        <p class="perk-desc">
                            Ponies tend to focus on your words and not your tone or theatrics. When
                            you increase
                ...
                {:else if roleName === 'Kindness'}
                    <div class="perk-item read-only">
                        ...
                        <p class="perk-desc">
                            Your greatest skill is understanding how other creatures feel. Skill
                            test is based on
                ...
                {:else if roleName === 'Laughter'}
                    <div class="perk-item read-only">
                        ...
                        <p class="perk-desc">
                            You live to spread cheer, but tapping into your own cheer reserves can
                            take a lot out of you. You have 2 Cheer Points. Cheer Points can be used
                            as a Free Action to reroll a failed Performance Skill Test.
                        </p>
                    </div>
                {:else if roleName === 'Loyalty'}
                    <div class="perk-item read-only">
                        ...
                        <p class="perk-desc">
                            You're quick to Spring into Action. You get ↑1 on Initiative. If you're
                            not first in Initiative order, you can roll a new Initiative once per
                            round.
                        </p>
                    </div>
                {:else if roleName === 'Magic'}
                    <div class="perk-item read-only">
                        ...
                        <p class="perk-desc">
                            You understand the magic of the universe, and can use it when you need
                            it. You have 2 Mystical Points to spend on: Essential Research,
                            Magically Fit In, Refocus, Spellcosting, Spellcialize.
                        </p>
                    </div>
                {/if}
            </div>
