...
import { CharacterStore, ESSENCE_SKILLS } from '../lib/store.svelte';
...
describe('CharacterStore serialization', () => {
    it('serialize/fromJSON round-trip preserves all fields', () => {
        const c = new CharacterStore();
        c.name = 'Twilight Sparkle';
        c.pronouns = 'she/her';
        ...
        c.movement = '40ft';
        c.diamondEssence = 'smarts';
        c.goldEssence = 'social';
        c.baseEssenceStr = 2;
        c.baseEssenceSpd = 3;
        c.baseEssenceSma = 4;
        c.baseEssenceSoc = 3;
        c.skills = { athletics: 2, alertness: 3 };
        c.specializations = { athletics: ['Running'] };
        c.influences = ['Education', 'Family'];
        c.hangup = 'Perfectionism';
        c.backgroundBonds = ['Bond 1', 'Bond 2'];
        c.generalPerks = ['Wealth'];
        c.generalPerkChoices = { 'Acute Sense': 'Sight' };
        c.magicRank = 2;
        c.masteredSpells = ['Light', 'Shield', '', '', '', '', '', ''];

        const json = c.serialize();
        const restored = CharacterStore.fromJSON(json, 'test-uuid-123');

        expect(restored.name).toBe('Twilight Sparkle');
        expect(restored.pronouns).toBe('she/her');
        ...
        expect(restored.goldEssence).toBe('social');
        expect(restored.baseEssenceStr).toBe(2);
        expect(restored.baseEssenceSpd).toBe(3);
        expect(restored.baseEssenceSma).toBe(4);
        expect(restored.baseEssenceSoc).toBe(3);
        expect(restored.skills).toEqual({ athletics: 2, alertness: 3 });
        expect(restored.specializations).toEqual({ athletics: ['Running'] });
        expect(restored.influences).toEqual(['Education', 'Family']);
        expect(restored.hangup).toBe('Perfectionism');
        expect(restored.backgroundBonds).toEqual(['Bond 1', 'Bond 2']);
        expect(restored.generalPerks).toEqual(['Wealth']);
        expect(restored.generalPerkChoices).toEqual({ 'Acute Sense': 'Sight' });
        expect(restored.magicRank).toBe(2);
        expect(restored.masteredSpells[0]).toBe('Light');
        expect(restored.masteredSpells[1]).toBe('Shield');
        expect(restored.id).toBe('test-uuid-123');
        expect(restored.locked).toBe(true);
    });

    it('fromJSON handles missing fields with defaults', () => {
        const json = JSON.stringify({ name: 'Minimal' });
        const restored = CharacterStore.fromJSON(json, 'id-1');

        expect(restored.name).toBe('Minimal');
        expect(restored.origin).toBe('');
        expect(restored.baseEssenceStr).toBe(1);
        expect(restored.skills).toEqual({});
        expect(restored.masteredSpells).toHaveLength(8);
        expect(restored.attacks).toHaveLength(3);
    });
    ...
    it('hasAnyInput detects user input', () => {
        const c = new CharacterStore();
        expect(c.hasAnyInput).toBe(false);

        c.name = 'Pony';
        expect(c.hasAnyInput).toBe(true);
